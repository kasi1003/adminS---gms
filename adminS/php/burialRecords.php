          ...
          <form>
            ...
            <div class="form-group">
              ...
              <select class="custom-select" id="inputGroupSelect02">
                ...
                <?php
                // If a CemeteryName is selected, fetch and populate the second select
                if ($selectedCemetery !== "") {
                    // SQL query to select SectionCode based on the selected CemeteryName
                    $sql = "SELECT SectionCode FROM sections WHERE CemeteryID IN (SELECT CemeteryID FROM cemeteries WHERE CemeteryName = '$selectedCemetery')";
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            // Output an <option> element for each SectionCode
                            echo '<option value="' . $sectionCode . '"';
                            if ($selectedSection == $sectionCode) {
                                echo ' selected';
                            }
                            echo '>' . $sectionCode . '</option>';
                        }
                    } else {
                        echo "No sections found for this cemetery.";
                    }
                }
                ?>
              </select>
